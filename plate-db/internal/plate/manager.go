...
func NewManagerClient(cfg Config, authCache *AuthCache, dbStore *DBStore, txns *TransactionManager) *ManagerClient {
	...
	go func() {
		...
		for {
			...
			case "delete":
				...
				plateID := strconv.Itoa(event.ID)
				m.txns.DeleteDB(plateID)
				if err := m.dbs.Delete(context.Background(), plateID); err != nil {
					Warn("failed to delete plate database:", err)
					continue
				}
				m.authCache.InvalidatePlate(plateID)
			default:
...
func (m *ManagerClient) respondToCreate(message []byte) error {
	...
	if rawData, ok := payload["data"]; ok {
		...
		if err := json.Unmarshal(rawData, &typed); err == nil && typed.ID > 0 {
			plateID = typed.ID
			if _, err := m.dbs.Ensure(context.Background(), strconv.Itoa(typed.ID)); err != nil {
	...
	if plateID > 0 {
		payload["id"] = json.RawMessage(strconv.Itoa(plateID))
	}
