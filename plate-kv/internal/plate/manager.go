...
func NewManagerClient(cfg Config, authCache *AuthCache, redisClient *redis.Client) *ManagerClient {
	...
	go func() {
		...
		for {
			...
			case "delete":
				...
				if err := m.deletePlateData(strconv.Itoa(event.ID)); err != nil {
					Warn("failed to delete plate data:", err)
					continue
				}
...
func (m *ManagerClient) deletePlateData(plateID string) error {
	if m.redis == nil {
		return errors.New("redis client is not configured")
	}

	ctx, cancel := context.WithTimeout(context.Background(), m.cfg.RedisOpTimeout)
	defer cancel()

	match := PrefixPattern(plateID, "*")
	var cursor uint64

	for {
		keys, next, err := m.redis.Scan(ctx, cursor, match, 500).Result()
		if err != nil {
			return err
		}
		if len(keys) > 0 {
			if err := m.redis.Unlink(ctx, keys...).Err(); err != nil {
				return err
			}
		}
		cursor = next
