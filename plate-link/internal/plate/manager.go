...
func NewManagerClient(cfg Config, authCache *AuthCache, redisClient *redis.Client, links *LinkStore) *ManagerClient {
	...
	go func() {
		...
		for {
			...
			case "delete":
				...
				if m.links != nil {
					if err := m.links.DeletePlateData(context.Background(), plateID); err != nil {
						Warn("failed to delete plate links:", err)
					}
				}
