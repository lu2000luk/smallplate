...
func LoadConfig() (Config, error) {
	cfg := Config{
		...
		HTTPAddr:        envString("HTTP_ADDR", ":3300"),
		DataDir:         envString("DATA_DIR", "data"),
		ChromaURL:       strings.TrimRight(strings.TrimSpace(os.Getenv("CHROMA_URL")), "/"),
		AuthCacheSize:   envInt("AUTH_CACHE_SIZE", 1000),
		OpTimeout:       envDuration("VEC_OP_TIMEOUT", 15*time.Second),
	...
	if cfg.ChromaURL == "" {
		missing = append(missing, "CHROMA_URL")
	}
	if len(missing) > 0 {
